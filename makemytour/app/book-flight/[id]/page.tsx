...
export default function BookFlightPage() {
  ...
  // TOP IN-LINE SEARCH AUTOCOMPLETE SUGGESTION STATES
  ...
  // Master list pool for autocomplete filters
  const standardCitiesPool = ["Delhi", "Mumbai", "Bangalore", "Kolkata", "Goa", "Shimla", "Paris", "Tokyo", "Davangere", "Bali", "New York", "London", "Dubai", "Singapore", "Sydney", "Indonesia", "France"];
  ...
  const handleAddNewReviewRecord = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Thank you! Your flight review has been published securely.");
      }
    } catch (err) {
        ...
        <form onSubmit={handleInPageFlightSearch} className="max-w-6xl w-full mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full p-2 rounded bg-slate-800 border border-slate-700 text-white outline-none font-bold text-xs" 
            ...
            {filteredSearchResults.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {filteredSearchResults.map((item, index) => (
                  ...
                  >
                    <div className="w-16 h-16 rounded-lg overflow-hidden border border-slate-700 flex-shrink-0"><img src={item.imageUrl} alt="Flight thumb" className="w-full h-full object-cover" /></div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 text-left">
            <div className="flex justify-between items-start border-b border-gray-100 pb-4 mb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">📅 Scheduled Departure • Non Stop • {flight?.timings || "03:41 PM ➔ 06:41 PM"} • Runs: {flight ? (flight.operatingDays || "Daily") : "Daily"}</p>
              </div>
          ...
          <div className="w-full bg-white border rounded-xl p-6 shadow-sm text-left space-y-5">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b pb-3">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium">Read traveler perspectives or contribute a review</p>
              </div>
