...
export default function ProfileDashboardPage() {
  ...
  // ADDED: State to track pinned flights from Radar page
  ...
  const fetchProfileData = () => {
    ...
    // ADDED: Load tracked flights from local storage automatically
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-gray-800 relative">
      ...
      <div className="max-w-6xl w-full mx-auto px-8 pt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div onClick={() => { setActiveBookingFilter("all"); setActiveAccountTab("profile"); setShowBookingsView(true); }} className={`p-5 rounded-xl border cursor-pointer transition-all shadow-sm flex items-center gap-4 ${activeBookingFilter === 'all' && showBookingsView && activeAccountTab === 'profile' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white hover:bg-slate-50 border-gray-200'}`}>
          <div className="text-xl">📋</div>
          <div>
            <h4 className="font-black text-sm">My Booking Lists</h4>
            <p className="text-[10px] opacity-70">Track paid active trip reservations</p>
          </div>
        </div>
        <div onClick={() => { setActiveBookingFilter("cancelled"); setActiveAccountTab("profile"); setShowBookingsView(true); }} className={`p-5 rounded-xl border cursor-pointer transition-all shadow-sm flex items-center gap-4 ${activeBookingFilter === 'cancelled' && showBookingsView && activeAccountTab === 'profile' ? 'bg-red-600 text-white border-red-600' : 'bg-white hover:bg-slate-50 border-gray-200'}`}>
          <div className="text-xl">❌</div>
          <div>
            <h4 className="font-black text-sm">Cancelled List</h4>
            <p className="text-[10px] opacity-70">Review historical voided elements</p>
          </div>
        </div>
        <div onClick={() => { setActiveBookingFilter("refunds"); setActiveAccountTab("profile"); setShowBookingsView(true); }} className={`p-5 rounded-xl border cursor-pointer transition-all shadow-sm flex items-center gap-4 ${activeBookingFilter === 'refunds' && showBookingsView && activeAccountTab === 'profile' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white hover:bg-slate-50 border-gray-200'}`}>
          <div className="text-xl">💰</div>
          <div>
            <h4 className="font-black text-sm">Refund Status</h4>
            <p className="text-[10px] opacity-70">Monitor payout details</p>
          </div>
        </div>
      </div>
        ...
        <div className="md:col-span-4 space-y-4">
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-4 text-left">
                ...
                <div className="space-y-1 flex flex-col">
                    <button onClick={() => { setActiveAccountTab("profile"); setShowBookingsView(false); }} className={`w-full text-left font-bold px-3 py-2.5 rounded-xl flex items-center gap-2 ${activeAccountTab === "profile" && !showBookingsView ? "bg-slate-900 text-white shadow-sm" : "hover:bg-slate-50 text-slate-600"}`}><User className="w-4 h-4" /> My Profile Details</button>
                    <button onClick={() => { setActiveAccountTab("ewallet"); setShowBookingsView(false); }} className={`w-full text-left font-bold px-3 py-2.5 rounded-xl flex items-center gap-2 ${activeAccountTab === "ewallet" ? "bg-slate-900 text-white shadow-sm" : "hover:bg-slate-50 text-slate-600"}`}><Wallet className="w-4 h-4" /> MakeMyTour Ewallet</button>
                    <button onClick={() => { setActiveAccountTab("password"); setShowBookingsView(false); }} className={`w-full text-left font-bold px-3 py-2.5 rounded-xl flex items-center gap-2 ${activeAccountTab === "password" ? "bg-slate-900 text-white shadow-sm" : "hover:bg-slate-50 text-slate-600"}`}><Lock className="w-4 h-4" /> Change Password</button>
                </div>
            </div>
        ...
        <div className="md:col-span-8 bg-white border border-gray-200 rounded-2xl shadow-sm p-6 space-y-6 text-left">
          {activeAccountTab === "profile" && !showBookingsView && (
            <div className="space-y-6 animate-in fade-in duration-100">
              <div className="border-b pb-2"><h3 className="text-base font-black text-gray-950 uppercase tracking-wide">My Personal Profile Details</h3></div>
          ...
          {activeAccountTab === "profile" && showBookingsView && (
            <div className="space-y-4 animate-in fade-in duration-100">
              ...
              {processedBookingsList.length === 0 ? <p className="text-slate-400 font-medium py-6 text-center border-2 border-dashed rounded-xl">No booking transaction profiles found.</p> : processedBookingsList.map((booking, index) => (
                <div key={index} className="border border-gray-200 rounded-xl p-5 bg-slate-50/40 space-y-4">
                  <div className="flex justify-between items-center">
                    <div className="flex gap-3 items-center">
                      ...
                      <div><h4 className="font-bold text-xs text-gray-900">{booking.type} Ticket Pass Summary</h4><p className="font-mono text-gray-400 text-[10px]">ID: {booking.bookingId}</p></div>
                    </div>
                    <span className={`font-bold px-2 py-0.5 rounded text-[9px] uppercase ${booking.cancelled ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{booking.cancelled ? 'Cancelled' : 'Paid & Active'}</span>
                  </div>
                  ...
                  {booking.cancelled && (
                    <div className="p-4 border border-red-200 bg-red-50/30 rounded-xl text-left space-y-1.5 font-medium text-slate-600">
                      <p className="text-red-700 font-black uppercase text-[10px] tracking-wide">❌ Cancellation Status</p>
                      <p>Refund Amount: <span className="text-emerald-700 font-black">{getDisplayPrice(booking.refundAmount || booking.totalPrice * 0.70, booking.currency || "INR")}</span></p>
                    </div>
                  )}
                  {!booking.cancelled && (
                    <div className="flex justify-end"><button onClick={() => handleOpenCancelModal(booking)} className="text-[10px] font-bold text-red-500 bg-white border border-red-200 hover:bg-red-50 px-3 py-1 rounded-lg transition-all">Process Cancellation</button></div>
                  )}
                </div>
              ))}
            </div>
          ...
          {activeAccountTab === "password" && (
             <div className="space-y-6">
              <div className="border-b pb-2"><h3 className="text-base font-black text-gray-950 uppercase tracking-wide">🔒 Account Credentials</h3></div>
              <form onSubmit={handleSecurityPasswordChange} className="space-y-4 max-w-sm">
                <div><label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Enter New Password</label><input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full border p-2.5 bg-slate-50 text-xs font-bold rounded-lg outline-none" /></div>
                <div><label className="block text-[11px] font-bold text-gray-500 uppercase mb-1">Confirm Password</label><input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full border p-2.5 bg-slate-50 text-xs font-bold rounded-lg outline-none" /></div>
                <button type="submit" className="bg-slate-900 hover:bg-black text-white font-bold px-4 py-2.5 rounded-lg uppercase text-[11px] tracking-wider">Save Credentials</button>
              </form>
            </div>
          )}
        </div>
      ...
      {isModalOpen && activeBooking && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl bg-zinc-950 p-6 border border-zinc-800 text-white text-left animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-black mb-1">Cancel Reservation</h3>
            <select value={cancellationReason} onChange={(e) => setCancellationReason(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 text-sm text-white mb-4">
              <option value="">-- Reason --</option>
              <option value="Change of plans">Change of plans</option>
              <option value="Health / Emergency">Health / Emergency</option>
            </select>
            {actionError && <p className="text-xs text-red-400 font-semibold mb-4">{actionError}</p>}
            <div className="flex justify-end gap-2.5">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-xs font-bold text-zinc-400">Cancel</button>
              <button onClick={handleConfirmCancellation} className="px-4 py-2 text-xs font-bold bg-red-600 rounded-xl">Confirm Cancellation</button>
            </div>
          </div>
        </div>
      )}
